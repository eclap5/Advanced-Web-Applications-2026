...
This week strengthens the concepts of authentication and authorization by adding RBAC (role-based access control) to the application and introducing input validation and sanitization within the context of authentication. The front-end introduces external libraries for UI components (Material UI), which demonstrates that we don't need to implement all UI components or stylings from scratch, and implements protected routes based on authentication and user roles. In addition the layered architecture is fully implemented in the scope of what is intended to teach in this course.
